...
class VerifyMobile extends Component
{
    ...
    public bool $isOtpExpired = false;
    ...
    private function calculateRemainingTime(): void
    {
        if ($this->user->mobile_otp_sent_at) {
            ...
            $secondsPassed = $lastSent->diffInSeconds(Carbon::now());
            if ($secondsPassed < 60) {
    ...
    public function sendOtp(): void
    {
        ...
        try {
            ...
            $this->isOtpExpired = false; // ریست وضعیت انقضا

            $this->user->save();

            $this->dispatch('otp-sent', seconds: 60);

            session()->flash('otp', 'کد جدید برای شما پیامک شد.');
...
//            curl_close($handler);
    ...
    {
        $enteredOtp = (string) $this->enOtp;
        $actualOtp = (string) $this->user->mobile_otp;

        $otpSentAt = Carbon::create($this->user->mobile_otp_sent_at);
        $isExpired = $otpSentAt->addMinutes(5)->isPast();

        if ($isExpired) {
            session()->flash('otp', 'کد شما منقضی شده است. کد جدید دریافت کنید.');
            session()->flash('color', 'text-red-500');
        ...
        if ($enteredOtp === $actualOtp) {
            ...
            session()->forget(['mobile', 'otp', 'color']);
    ...
    {
        return view('livewire.auth.verify-mobile')
            ->layout('components.layouts.guest')
            ->title('تایید شماره موبایل - نویسینو');
    }
}