            ...
            <div class="sm:hidden">
                ...
                @if((is_null($product->variant) && $product->stock > 0) || (!is_null($product->variant) && $product->variants->sum('stock') > 0))
                    <div class="flex items-center gap-2 text-lg bg-gray-50 px-3 py-2 rounded-lg mb-8">
                        ...
                        @if($hasDiscount)
                            <span class="line-through text-gray-400 text-sm">{{ english_to_persian_num(number_format($basePrice)) }}</span>
                            <span class="font-bold text-pars-700 text-lg">{{ english_to_persian_num(number_format($finalPrice)) }}</span>
                        @else
                            <span class="font-bold text-pars-700 text-lg">{{ english_to_persian_num(number_format($finalPrice)) }}</span>
                        @endif
                        <span>تومان</span>
