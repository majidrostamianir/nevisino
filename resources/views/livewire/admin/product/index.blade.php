                ...
                <tr class="border-b border-gray-200 transition-colors hover:bg-gray-50 {{ $loop->even ? 'bg-gray-50/50' : 'bg-white' }}">
                                   ...
                                   title="مشاهده در ترب">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 88 88">
                                        <g fill="none">
                                            ...
                                            <path fill="#FF8C00"
                                                  d="M31.969 10.22c-10.91 1.08-20.463 5.721-24.054 11.191C-5.195 41.216.3 67.853 20.193 80.905l.73-1.112c3.191-4.847 2.735-11.055-.799-15.322v-.045c-8.22-10.079-9.36-24.72-1.835-36.16C26.61 15.67 42.685 11.22 56.114 17.031c0 0-11.4-8.059-24.122-6.81h-.023z"></path>
                                            <path fill="#FFA500"
                                                  d="M62.544 22.32s-.342-4.086 1.482-7.037c1.824-2.951 5.7-5.845 10.602-5.539 4.925.307 5.723 1.442 5.723 1.442s.114 3.972-1.254 6.242-4.218 5.606-8.322 6.446c-4.104.829-8.231-1.555-8.231-1.555z"></path>
                                            <path fill="#FFB347"
                                                  d="M61.062 21.173s-.456-3.972 2.28-7.944c2.736-3.973 5.7-4.563 5.7-4.563S68.7 3.218 66.078.608c.228-.057-3.124.08-6.042 3.291-2.577 2.815-4.104 7.264-2.964 11.577 1.14 4.312 3.99 5.697 3.99 5.697z"></path>
                                        </g>
                                    </svg>
                                </a>
                            @endif
                    ...
                    <td class="px-4 py-3">
                        ...
                        <div class="text-xs text-gray-500 flex flex-col">
                            <span>قبلی: {{ english_to_persian_num(number_format($previousPrices[$product->id]['bulk_price_previous'])) }} تومان</span>
                            <span class="text-[10px] text-gray-400">
                                {{ english_to_persian_num(verta($priceUpdates[$product->id]['bulk_price_updated_at'])->format('H:i - Y/m/d')) }}
                            </span>
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex flex-col gap-1">
                            <small class="text-blue-400">{{ english_to_persian_num(number_format($price_for_show[$product->id])) }}</small>
                            <input type="text"
                    ...
                    <td class="px-4 py-3">
                        <div class="flex flex-col gap-1">
                            <small class="text-blue-400">{{ english_to_persian_num(number_format($installment_price_for_show[$product->id])) }}</small>
                            <input type="text"
                            ...
                            {{-- نمایش قیمت قبلی --}}
                            <div class="text-xs text-gray-500 flex flex-col">
                                <span>قبلی: {{ english_to_persian_num(number_format($previousPrices[$product->id]['installment_price_previous'])) }} تومان</span>
                                <span class="text-[10px] text-gray-400">
                                    {{ english_to_persian_num(verta($priceUpdates[$product->id]['installment_price_updated_at'])->format('H:i - Y/m/d')) }}
                                </span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex flex-col gap-1">
                            ...
                            {{-- نمایش قیمت قبلی --}}
                            <div class="text-xs text-gray-500 flex flex-col">
                                <span>قبلی: {{ english_to_persian_num(number_format($previousPrices[$product->id]['discounted_price_previous'])) }} تومان</span>
                                <span class="text-[10px] text-gray-400">
                                    {{ english_to_persian_num(verta($priceUpdates[$product->id]['discounted_price_updated_at'])->format('H:i - Y/m/d')) }}
                                </span>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-4 py-3">
                        <div class="flex flex-col gap-1">
                            ...
                            <div class="text-xs text-gray-500 flex flex-col">
                                <span>قبلی: {{ english_to_persian_num(number_format($previousPrices[$product->id]['discounted_installment_price_previous'])) }} تومان</span>
                                <span class="text-[10px] text-gray-400">
                                    {{ english_to_persian_num(verta($priceUpdates[$product->id]['discounted_installment_price_updated_at'])->format('H:i - Y/m/d')) }}
                                </span>
                            </div>
                        </div>
                    </td>
